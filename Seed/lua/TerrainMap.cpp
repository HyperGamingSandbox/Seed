...
void TerrainMapScaleBlock::__init(lua_State *L) {
	...
	lint size = ((lint)1) << ((_level + 1) * B_BITS);
	_xe = _x + size - 1;
	_ye = _y + size - 1;
...
size_t TerrainMapScaleBlock::S_getSize() {

	/*
	unsigned short int _level;
	unsigned short int _m; -> 2

	lint _x, _y, _xe, _ye; -> 4
	*/
	
	size_t sz = sizeof(_x) * 4 + sizeof(_level) * 2;
...
size_t TerrainMapScaleBlock::S_store(void *data) {

	// auto sz = S_getSize();
	// void *data = new char[sz];
...
size_t TerrainMap::S_store(void *data) {
...
LuaEMethod(TerrainMap, store) {

	auto sz = S_getSize();
	void *_data = new char[sz];

	size_t chunk_sz = S_store(_data);
...
LuaMethods(TerrainMap) = {
	...
	// LuaMethodDesc(TerrainMap, store),
	{ 0,0 }
};
